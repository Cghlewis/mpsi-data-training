...
**First rule**: Never touch the raw data. You never know when you will make an error and need to return to the the uncorrected data file. 
...
**Second rule**: Keep everything (except variables that are identifying and completely unnecessary for project). Keep both item level and aggregated variables, recoded and original variables.

**Third rule**: Always de-identify the clean/analysis data. Ensure your study ID is in the data and is accurate, then remove all PII (name, SSN, email, DOB, school name, parent name, IP address, etc.).
...
1. Read in data
    + Whether this is one file from a folder on your computer, or multiple files, or calling data from an API
2. Review your data
...
4. If your data does not already contain a study ID, read in a roster from your participant tracking and join your roster with your data
...
10. Standardize
    ...
    + Rescale variables
    + Collapse open ended variables into categorical variables (ex: school a, SchoolA, school b, School B, into School A, School B)
11. Recode variables
...
12. Calculate variables laid out in the data dictionary
    + Create sum and mean scores
    + Create any necessary standardized scores
...
14. Validate data
    + Adding [flags](https://esajournals.onlinelibrary.wiley.com/doi/full/10.1890/0012-9623-90.2.205) (columns that identify issues in a row) can be helpful 
    ...
    + Flag rows with issues (1 = issues, 0 = no issue)
      ...
      + Do cross validation for impossible values
...
Over the years I have seen researchers manually clean data in programs like Excel and in every scenario I can think of, it's a bad idea for the following for two major reasons: a) It is not reproducible, b) It leads to errors, and c) It is time consuming. My recommendation is to clean your data, no matter how small the task, with syntax. Even if you use a graphical user interace (GUI) program like SPSS (point and click), you need to log every move you make in that program and paste the code into a syntax file. Syntax files allow you to have a record of every transformation you make from the raw data to your clean data. While writing syntax may seem time consuming up, it helps you to be more thoughtful in your data cleaning process and it can save you an enormous amount of time in the future if you plan to clean data with the same features multiple times (in say a longitudinal study).
...
1. Working with a partner who sends you clean data, you find three small errors in the data they sent us. 
...
These all seemed like quick changes you could make very easily in a point and click interface, so you did. Then you saved that dataset as the final clean dataset. However, a month later, your partner found an error in the data they sent you, and they sent you a revised clean dataset. 

This is the process you had to go through:  
