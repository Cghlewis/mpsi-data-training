...
The simplest way to maintain participant privacy, is to collect anonymous data. If you collect anonymous data, no participant identifying information will be collected in your instruments (name, email, date of birth, etc.). Importantly, you will also want to make sure to not collect IP addresses as they can be used to identify an individual's computer.

More commonly in education research though, you will be collecting some identifiers and will be assigning unique IDs to all participants to maintain their [confidentiality](https://www.etsu.edu/irb/documents/confidentiality_vs_privacy.pdf) in your research data. In longitudinal studies in particular, you will be using that unique participant ID to link data across time. To the best of your ability, all of your data collection instruments should have that study ID associated with it, rather than name, to ensure confidentiality as well as secure accurate data. This is especially important for longitudinal studies, where maintaining the accuracy of your study IDs is critical to linking data over time.

### Web-based Data:

When sending out or completing in-person web-based surveys/assessments, you can do one of the following (this is not all-inclusive):  
  ...
  1. Send/give individual links to participants (where each survey link is connected to a study ID within the system). This can usually be done easily in a survey platform using a *panel*. 
    + This is the most error-proof way to ensure study IDs are entered correctly. You load in IDs directly from your tracking database so that when data is downloaded, the correct ID is already linked to each participant and name is never attached to the data.
  ...
  3. Send one link out to all participants and have each participant enter their name and other identifying information. 
      + This may be the most problematic out of all the options in terms of data management, privacy, and potential for errors. 
      + You options for working with this data are:
...
If you take paper forms into the field consider doing the following:  
  ...
  * Write the study ID on the form and then use either a removable label with participant name and place that over the ID or attach a cover sheet with identifying information such as student name. When you return to the office, you can remove the name label and be left with only the ID on the form.
    + It is this ID only that you will enter into your database, no name.
    ...
    + Double and triple check that the study ID is correct *before* removing the label or cover sheet
    + Make a plan for the labels/cover sheets (either shred them if they are no longer needed, or store them securely in a locked file cabinet)

  * If you need a roster of consented participants in the field with both name and study ID (say to find students and then enter their ID into a data collection system, such as an observation instrument), you can do one of two things:
...
A tracking database can be set up in software or an app such as:
...
When setting up your tables consider the following:
  ...
  + Decide if you want to keep demographic type information in a table separate from data collection information
    - Typically the only demographic information you will keep in your tracking database is information you collect that is considered identifiable and therefore you do not keep it in your study data (ex: DOB for scoring assessments, state ID for linking to state education data, student email for sending out gift cards). 
    - If the project coordinator is the only one using the tracking database, it is fine to have demographics embedded in the data collection table
    - However, if many team members are tracking data, you may choose to have a *student-demographics* table and a *student-data-collection* table in order to minimize the number of people accessing identifiable participant information
    - Additionally, if you plan to keep your different waves of data collection in separate tables, it will be beneficial to have a separate *student-demographics* table so that there is no repeating information across waves of data collection (ex: repeating DOB across tables).
  + Make sure to include both primary and foreign keys in all tables in relational databases
...
Fields to consider tracking in your database at the beginning of your study include:
  ...
  + Consent rate information (# of consents sent out)
