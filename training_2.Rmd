...
### Offline Data:
  ...
  + If you take paper forms into the field with participant names on them, consider doing the following to link those names to IDs at a later time:  
    - Write the study ID on the form and then use a removable label with participant name and place that over the ID. That way the participant never sees the ID, and when you return to the office, you can remove the name label, shred it, and be left with only the ID on the form.  
    - Attach a cover sheet to each form that includes identifying information such as student and/or teacher name, then when returning to the office, remove the cover sheet, shred it, and write the study ID on the form before entry.   
    - If you need the name in the field (to identify students), but you also need the ID in the field (say to enter on a data collection form) then consider having one roster with names with a double ID (such as 1,2,3,4) and another sheet that connects the double ID to the true study IDs. These two lists could obviously also be housed on a password protected tablet or phone in the field for additional security. 
