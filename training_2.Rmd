---
title: "Data Collection Structures"
output: 
  html_document:
    css: "css/custom.css"
    toc: TRUE
...
There are many web-based survey platforms (Ex: Qualtrics, Survey Monkey, Google Forms, Microsoft Forms, SurveyCTO) you can use to develop surveys. If those existing platforms do not meet your needs, you can always build your own web-based application to collect data. 
  ...
  + You can build your own application to collect data in tools such as R (ex: using [{formR}](https://formr.org/documentation#get_started)) or Tableau
...
No matter what platform you use, building your survey requires some up front planning. First and foremost make sure to build a survey that is both valid and reliable. Secondly, take time to think how the data you collect will be *translated into a database*. Remember, every question answered is then stored in a database within the platform that you will later download to a file. Here are several suggestions to adhere to when creating your survey that will provide you with more accurate data and reduce future data management headaches (*While I will not provide a tutorial for how to implement each suggestion, they are all possible in a platform such as Qualtrics*):  
  ...
  1. If there is a finite number of response options, and the number isn't too large (less than ~ 20) use a drop-down list rather than open-ended field
      ...
      - An exception to this rule is for categories such as demographics. For example, for gender you may prefer to allow people to enter their gender in an open text box rather than giving them a pre-determined list, there are tools, such as the R package called [gendercoder](https://ropenscilabs.github.io/gendercoder/), that help you recategorize this data through coding after it is collected (we will talk about these tools more in later trainings).
...
If your site or participants do not have WiFi and/or email access, you generally have a few options.
  ...
  1. One option is to work with a survey platform (such as Qualtrics or SurveyCTO) that allows you to create a survey online, administer it using an offline survey app on your phone or tablet, and then upload that data back to the survey platform once you have an internet connection again.

  2. Still, depending on your study, participants, and resources, building a web-based survey may not be possible and/or it may not be the best option for your project. So your second option is a paper survey. 
...
If WiFi is not available to you in the field, consider making an electronic form that still eliminates the need for data entry (so not a paper form) and that ideally connects to or creates some type of database or table. Then have data collectors enter data into this form on a device in the field. This does not have to be high-tech. It could simply be: 
  ...
  1. An Excel file that is pre-built with every observation field and the options for each field are limited to only the correct options through a drop-down list.
  2. Create a Microsoft Access form that is connected to a database
  3. Utilize the offline survey options provided by tools such as Qualtrics (see Offline Surveys section)
  4. Build or use existing offline tools. For duration code classroom observations, we've used [software](https://mooses.vueinnovations.com/) built by a developer at Vanderbilt to collect frequency and duration observation data. It stores observation text files to the data collectors device. 
...
Most studies require researchers to collect consent and/or assent on participants. Informed consent is collected on any participant, 18 years or older. If a student is under 18 years old, a parent consent as well as a student assent is collected. Consents and assents are typically collected one of two ways.
  ...
  1. Web-based: Consents and assents can be collected through secure web-based tools, such as Qualtrics or DocuSign. Consult with your IRB to see what tools are approved. 
...
If you end up having to use paper forms to collect any of your data and need to manually enter that data, then there are several steps you can take to minimize error in that process.
  ...
  1. Set up databases for staff to enter the data  
  + Consider using a database tool such as Access, FileMaker, RedCap, or QuickBase rather than a spreadsheet. Most of these tools allow you to set up forms for users to enter data into that are then submitted into a table on the back end. This reduces data entry errors.
  + If you must use a spreadsheet tool such as SPSS or Excel, be aware of some of the limitations and possible issues: 
    - The formatting that Excel sometimes applies to fields may cause errors in your data (dates get formatted as numeric, strings get formatted as dates, leading zeros get removed from ID variables who should be formatted as character but Excel converts them to numeric, and other nightmares)  
  ...
  2. Develop a data entry protocol. Your protocol can/should include:  
  ...
  + Set up a system for error checking  
      - I highly recommend you implement a system where you [double enter](http://barchard.faculty.unlv.edu/doubleentry/Scott%20Data%20Entry%20WPA%202008.pdf) a portion, if not all of your data, to check for entry errors.  
        ...
        - Then when all data is entered in both forms, the following things happen:  
          a) The project coordinator checks both forms/files to make sure the N of each file matches what their participant tracking database says they collected (i.e. - someone didn't forget to enter a form)  
          b) Then a syntax is run to check for errors (I will discuss this more in later trainings)  
          c) If errors are found, they are rectified by going back to the original databases and correcting the error in database 1 or database 2, wherever the error occurred. Then the syntax is run again until no more errors are found  
          d) Then both files are accurate and either database 1 or database 2 can be used as your analysis file.  
            
   📑 Reynolds and Schatsnieder from Florida State University have some great suggestions [here](https://figshare.com/articles/preprint/The_Basics_of_Data_Management/13215350/1) for setting up a data entry station and protocol for any manual data entry that needs to occur.  
   📑 Data Management Episode 1 and 2 of [Within and Between](http://www.withinandbetweenpod.com/) podcast has some great examples of how to set up data entry protocol.  
   📑 While I haven't read [The Practice of Survey Research](https://methods.sagepub.com/Book/the-practice-of-survey-research), it looks to have an entire chapter devoted to data entry that may be worth reading  
   📑 Read [Data Organization in Spreadsheets](https://www.tandfonline.com/doi/full/10.1080/00031305.2017.1375989) to understand how data should and should not be organized if you use a spreadsheet tool for data entry.   
   📑 Barchard & Verenikina have an article: [Improving data accuracy: Selecting the best data checking technique](https://www.sciencedirect.com/science/article/abs/pii/S0747563213000873)   
   📑 Barchard & Pace have another article: [Preventing human error: The impact of data entry methods on data accuracy and statistical results](https://www.sciencedirect.com/science/article/abs/pii/S0747563211000707) 
...
More commonly in education research you will be collecting some identifiers (ex: name). However, in order to maintain participant [confidentiality](https://www.etsu.edu/irb/documents/confidentiality_vs_privacy.pdf) you cannot keep those identifiers in your research data. You will need to assign unique IDs to all participants and replace all identifying information with that study ID. In longitudinal studies in particular, you will be using that unique participant ID to link data across time and across measures. 
...
When sending out or completing in-person web-based surveys/assessments, you can do one of the following to connect your data to a participant (this is not all-inclusive):  
  ...
  1. Send/give individual links to participants (where each survey link is connected to a study ID within the system). This can usually be done easily in a survey platform such as Qualtrics using a [contact list](https://www.qualtrics.com/support/survey-platform/contacts/contact-list-overview/).
  ...
  1. Send one link out to all participants and have each participant enter their name and other identifying information. I say "and other identifying information" because what the participant enters needs to be unique and not duplicated. Some people in your study may have the exact same name as someone else in your study. To prevent causing confusion, collect two pieces of information such as name and DOB so you are able to link that individual to the correct study ID.
...
1. Not posting the link on social media
1. Using [captcha verification](https://www.qualtrics.com/support/survey-platform/survey-module/editing-questions/question-types-guide/advanced/captcha-verification/)
1. Do not automate payment upon survey completion
1. Include open-ended questions
1. Build attention/logic checks into survey
1. Ask some of the same questions twice (once early on and again at the end)
...
Resources:
  ...
  📑 [Melissa Simone, Behavioral Scientist](https://behavioralscientist.org/how-to-battle-the-bots-wrecking-your-online-study/)   
  📑 [Melissa Simone, STAT](https://www-new.statnews.com/2019/11/21/bots-started-sabotaging-my-online-research-i-fought-back/)   
  📑 [Cloud Research](https://www.cloudresearch.com/resources/blog/after-the-bot-scare-understanding-whats-been-happening-with-data-collection-on-mturk-and-how-to-stop-it/)    
:::
...
Relational database resources:
  ...
  📑 [7 Excel Spreadsheet Problems](https://www.alphr.com/business/1000859/7-spreadsheet-problems-and-how-to-solve-them/)  
  📑 [Jenna Jordan has a great example of how and why to build a relational database](https://drive.google.com/file/d/11QlIriyQCCu0NYbh4OUf6D1_odBxGVb7/view)  
  📑 [QuickBase explanation of table-to-table relationships](https://help.quickbase.com/user-assistance/about_relationships.html)  
  📑 [Relating tables in Google Tables](https://support.google.com/area120-tables/answer/10400909?hl=en)  
  📑 [Airtable vs Google Sheets](https://brycevernon.com/airtable-vs-google-sheets-comparison/)  
...
There are many other ways to load data into a database but for this use case (tracking participation in an education research study) these are probably the most common methods. However, you can read about several other methods [here](https://www.quackit.com/database/tutorial/adding_data_to_database.cfm). 

Also, most recently I've heard people mention the idea of automating at least some data entry/tracking through the use of unique scannable codes **or** through integration of your online data collection platform and your tracking database (ex: [Qualtrics and Quick Base](https://www.workato.com/integrations/quickbase+qualtrics)). This type of tracking most likely requires some technical skills to set up this linking process. But it's a cool idea and may reduce at least some error that occurs in manually tracking data. For example, by creating a unique QR code/barcode (linked to a participant ID) that is added to every paper survey, similar to what is used for [tracking inventory](https://blog.beaconstac.com/2020/07/qr-codes-for-inventory-management/), you can scan that code when data collection is complete to "check in" a survey to your participant database. Or by integrating your survey platform and participant database, you could add new participants to your database as they complete their consents and surveys online.

QR code/barcode resources:

  📑 [Creating Barcodes in Excel](https://www.smartsheet.com/content/excel-barcodes)   
  📑 [Scanning Barcodes into an Excel Spreadsheet](https://www.youtube.com/watch?v=yssnGacoTNA)   
  📑 [Creating Barcodes in Word](https://support.microsoft.com/en-us/office/add-barcodes-to-labels-495cf789-c9d9-4ea5-8eb9-7299170d4831)   
  📑 [Scanning QR codes into Google Sheets](https://www.youtube.com/watch?v=XGS3ctyVbHA)   
  📑 [Creating Barcodes in Access](https://www.activebarcode.com/howto/access-barcode-object-report.html)   
  📑 [Scanning Barcodes into Access](https://www.techwalla.com/articles/how-to-read-barcode-data-into-ms-access-databases)   
  
Integration resources:

  📑 [Examples of Qualtrics Integration](https://www.qualtrics.com/support/integrations/integrations-overview/)   
  📑 [Examples of QuickBase Integration](https://www.quickbase.com/business-application-platform/integrations)   
...
Because your participant database contains identifiable information about your participants, this data must have the highest security. The specifics of this security will be set by your university or institution. However, the general rules for storing this type of data is:
  ...
  + It should be stored separately from your confidential study data (separate folder)
...
Your typical study data files are in formats such as .csv, .tsv, .xlsx, .txt, .docx, or a statistical program file such as .sav, .dta, .rds, or .sas. No matter the file type, your study data should all be de-identified/confidential, only including study IDs and no other identifiers. However, there are still some security precautions you should take.
  ...
  + Extant data you acquire, such as student record data from schools and districts may have its own set of regulations such as [FERPA](https://studentprivacy.ed.gov/audience/researchers) and [HIPAA](https://studentprivacy.ed.gov/sites/default/files/resource_document/file/ferpa-hipaa-guidance.pdf) so it is worth checking that your storage meets those requirements as well.
...
A newer type of data collection is occurring since the COVID-19 pandemic, and that is observations, interviews, and focus groups occurring via video conferencing. This data is especially sensitive as it may include names and faces as part of the recording. Again, you will want to refer to your specific institution guidelines, but generally:
  ...
  + You will want to make sure that software and cloud storage is HIPAA and/or FERPA-compliant if you need it to be
...
Paper data can be one of the most difficult data types to securely manage and track the storage of. When you have a distributed team, collecting data at many sites, sometimes in different states, it can be difficult to track the path and storage of physical data. You will want to make sure you have a very detailed plan for the storage of your data added to your protocol. Never play fast and loose with your paper data. Losing physical files can lead to breaches in confidentiality, loss of data, and other negative consequences. This holds true even for consents and assents. Even though those forms may not have analyzable data, you never know when your IRB may conduct an audit for participant consent, or if you may need to present them for future data requests with an entity such as a school district. If you lose consents/assents, it can look like you are collecting data on those who did not agree to be in the study.  
  ...
  + Never store paper data at a personal residence or leave in your vehicle. Always make sure your paper data makes it back to your office (or wherever you store your data). Have a rule in your protocol that says the data must always be returned to storage by the end of the day. 
    - An exception to this may be people collecting paper data out of state (from your office). In this case you will need a protocol for this data, such as:
      - Keeping the data in a locked file cabinet at the data collector's home until the data can be returned to the office  
      - Or scanning the forms and uploading to a secure folder at the end of every day and then shredding the original files  
...
When sharing data, if you data includes any form of PII, choose a secure method of file sharing. 

  + Secure methods of file transfer
    - Upload files to HIPAA/FERPA compliant cloud storage that is approved by your institution
    - Email an encrypted file, sharing the password separately
    - Mailing encrypted files stored on an encrypted device
    - Sharing data through an institution approved survey platform with encryption features
  + How **not** to share data:
    - Emailing an unencrypted file
      - This includes a password-protected Excel file
    - Mailing unencrypted media (ex: flash drives)

Resource: [J-PAL](https://www.povertyactionlab.org/sites/default/files/Data_Security_Procedures_December.pdf)
...
Again, I am not an IT professional, but some general security rules to keep in mind for all data and devices are:
  ...
  + There are [data classification levels (DCL)](https://www.umsystem.edu/ums/is/infosec/classification-definitions) that help you identify the security you need based on the type of data you have. Knowing what level your data falls under can help you start conversations with your IT department to decide if you are meeting the level of security you need to meet FERPA and HIPAA requirements if applicable.
...
Resources on data security:
  ...
  📑 [University of Pittsburgh, RDM](https://pitt.libguides.com/managedata/storing)  
  📑 [Veeam](https://www.veeam.com/blog/how-to-follow-the-3-2-1-backup-rule-with-veeam-backup-replication.html)  
  📑 [University of Connecticut](https://security.uconn.edu/data-security-checklist/#)  
  📑 [Brandeis University](https://www.brandeis.edu/ora/hrpp/pdfs/data-management.pdf)   
  📑 [Reasons to consider SharePoint for data storage](https://www.youtube.com/watch?v=RmQylg5_WSY)   
  📑 [Reasons to consider Institutional Servers for data storage](https://www.povertyactionlab.org/sites/default/files/Data_Security_Procedures_December.pdf)   
