...
In general though, some useful guidelines for setting up directory structure include:
  ...
  + Make your folder names machine readable
    ...
    - Similar to file names, consider using *_* to separate metadata pieces and *-* to separate words within the same piece of metadata
...
If you are like me, or most of humanity, you are guilty of giving files names like `research report final.docx` and `research report final-2.docx` and `research report-2-dan edits.docx`. It happens, but as you can see this can makes it very confusing to know any details about what these documents are? What is this research report on? What is the actual final version? When were the edits made on these documents? What are the differences between these documents?

This is why a file naming style guide is so important. It sets the protocol for you and your entire team to use streamlined, descriptive, organized and ultimately human **and** machine-readable file names to remove any future confusion.

General file naming conventions to follow include:

- Never use spaces between words. They can often break a URL when shared.
- Never use special characters. They can have meanings with programming languages and can cause problems.
- Never use periods. Again it can cause issues when using programming languages that look for the `.` before file extension
- Use `-` between similar elements and `_` between separate elements. This not only helps to make the name human readable but also allows your computer to read and search files easier.
...
- Pick a certain order for file name metadata to follow and stick to it. Ex:
...
3. Never use periods. 
4. Use `-` between similar elements and `_` between separate elements
5. Use the following metadata file naming order:
    - Order of use (if relevant--and always add a 0 before single digits)
    - Project
    - Time period (if relevant)
    - Participant
...
7. If there are multiple versions of a document on the same date, version using v# with a leading 0.
7. Only use lower case letters
8. Keep names under 45 characters
9. Choose abbreviations to use for common names/phrases and add them to your style guide
  - Project A Year 1 (2018-19) = y1
  - Project A = pa
  - student = s
  - teacher = t
  - survey = svy
7. Examples of this format:
    - `01_pa_y1_s_svy_cleaning-syntax_2021-01-22.R`
    - `02_pa_y1_s_svy_cleaning-syntax_2021-01-21v01.R`
    - `pa_s_svy_protocol_2020-10-01.docx`
    - `pa_y1_t_svy_data_raw_2020-08-03.csv`
...
In addition to best practices, here are just other practices that I've noticed others implement. You can pick and choose what works best with you. Just make sure whatever you choose, you put it in your style guide and follow those guidelines all throughout your project.
...
You do not need to have this decided right at the beginning of your project. You really don't even need to decide how to account for time until you are ready to start merging data across time. Until then, most likely you will know when each piece of data is collected by how you name that file and/or the name of the folder you placed it in.
...
1. If you decide to merge your data in wide format, you will need to concatenate time to your variable names in order to differentiate the difference between *toca1* collected in the fall and *toca1* collected in the spring. What this exactly looks like is all dependent on your longitudinal study design. But a few things to consider are:
    - How do you want to account for time?
      + Consider how your data is collected? 
        - Is a a cohort study? How many waves of data collection are there per cohort?
        - Or do you follow the same participants for the entire study
    - Do you prefer a prefix or suffix on your variables for time?
    - Do you want to use a number system or characters to indicate time?
    - One example of adding time to variable names:
      + *s1_scale_sum*
        + This would be student survey (s), wave 1, for the scale sum
          + Referring to my documentation I would know that wave 1 is the fall of 2019-20
...
2. If you decide to append data in long format over time then you no longer need to worry about having duplicate variables in a row. However, you will need to create time variables in order to capture the aspect of time. You will need to consider similar issues as we covered above:
    - How do you want to account for time?
      + Consider how your data is collected? 
        - Is a a cohort study? How many waves of data collection are there per cohort?
        - Or do you follow the same participants for the entire study
    - Do you want your time variables to be numeric or character variables?
    - One example of adding time variables:
      + *cohort* (1,2)
      + *wave* (1,2,3,4), with the 6 waves of data collection repeating for every cohort
        + These variables would allow me to know the year and month every data collection period
        + Documentation would tell me that 1=fall of recruitment year, 2=spring of recruitment year, 3=fall of follow-up year and 4=spring of follow-up year
        + And, while cohort is my grouping/randomization variable, by referring to my documentation I would also know that cohort 1 was recruited fall 2019-20 and cohort 2 was recruited in 2021-22.
        + With those two pieces of information (cohort and wave) I would know that cohort 1, wave 2 occurred in spring 2020.
...
Here is an example of how you would explain your variable naming in your documentation: 
