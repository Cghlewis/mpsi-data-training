...
- If a variable is a part of a scale, consider naming is an abbreviation of that scale plus the scale item number (add the number last to make data manipulations easier)
  + *toca1*
- Don't name a variable any keywords (such as `if`,  `for`, `repeat`) or functions used in any programming language (see example list of all base R functions [here](https://stat.ethz.ch/R-manual/R-devel/library/base/html/00Index.html)).
